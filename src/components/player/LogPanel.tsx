    ...
    <section className="onyx-log min-w-0 rounded-sm border border-hairline bg-background">
      <header className="flex flex-wrap items-center gap-2 border-b border-hairline bg-panel px-3 py-2 sm:px-4">
        <button
          ...
          className="label-machined flex min-h-10 items-center gap-2 text-foreground"
        >
          ...
          <div className="flex w-full flex-wrap items-center justify-end gap-1 sm:ml-auto sm:w-auto sm:flex-nowrap">
            {LEVELS.map((level) => (
              <button
                ...
                className={cn(
                  "readout min-h-9 rounded-sm px-2 py-1 text-[10px] uppercase tracking-widest transition-colors",
                  filter === level.key
      ...
      {open && (
        <div
          id={contentId}
          className="readout max-h-56 overflow-auto px-3 py-3 text-[10px] leading-relaxed sm:max-h-64 sm:px-4 sm:text-[11px]"
        >
          {visible.length === 0 ? (
              ...
              {visible.map((entry) => (
                <li key={entry.id} className="flex gap-3">
                  ...
                  <span className={cn("min-w-0 break-words", LEVEL_COLOR[entry.level])}>
                    ...
                    {entry.detail && (
                      <span className="block break-all text-muted-foreground">{entry.detail}</span>
                    )}
                  </span>
                </li>
              ))}
